...
class TestConsultantAgentCoreFeatures:
    ...
    @pytest.mark.asyncio
    async def test_should_answer_tea_related_questions(self):
        """测试：应该能回答茶艺相关问题"""
        ...
        for question in tea_questions:
            ...
            tea_keywords = ["茶", "tea", "效果", "好处", "放松", "提神", "茶艺"]
            has_relevant_content = any(keyword in response for keyword in tea_keywords)
            assert has_relevant_content, f"回答应该与茶艺相关，问题：{question}，回答：{response[:200]}..."
    ...
    async def test_should_search_knowledge_from_database(self):
        ...
        if len(knowledge_results) > 0:
            ...
            tea_keywords = ["茶", "tea", "效果", "好处", "提神", "放松"]
            has_relevant = any(keyword in all_content for keyword in tea_keywords)
            assert has_relevant, f"检索的知识应该与茶艺相关，但得到：{all_content[:300]}..."
    ...
    async def test_should_generate_professional_response(self):
        ...
        assert not response.startswith("抱歉"), f"对于茶艺相关问题不应该道歉开头：{response[:100]}..."
        assert "不知道" not in response, f"专业回答不应该说不知道：{response[:100]}..."

        professional_terms = ["提神", "放松", "清爽", "生津", "肠胃", "陈化"]
...
class TestConsultantAgentEdgeCases:
    ...
    async def test_should_work_with_stream_mode(self):
        ...
        tea_keywords = ["茶", "好处", "效果", "提神", "放松"]
        stream_has_content = any(keyword in stream_response for keyword in tea_keywords)
        normal_has_content = any(keyword in normal_response for keyword in tea_keywords)
