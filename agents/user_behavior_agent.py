...
class UserBehaviorAgent:
    ...
    def get_user_analysis(self, user_id: str = "default_user") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                'favorite_tea_master_id': preferences.get('favorite_tea_master_id'),
    ...
    async def generate_personalized_reminder(self, user_id: str = "default_user",
        ...
        try:
            ...
            if not analysis or not analysis.get('favorite_tea_master_id'):
            ...
            tm_db = TeaMasterDBRouter()
            tm_info = tm_db.get_tea_master_by_id(analysis['favorite_tea_master_id'])

            tm_name = tm_info.get('name', '您偏爱的茶艺师') if tm_info else '您偏爱的茶艺师'
            tm_specialty = tm_info.get('specialty', '') if tm_info else ''
            service = analysis.get('favorite_service', '品茶')
            duration = analysis.get('favorite_duration', 60)

            self.logger.info(f"茶艺师信息: {tm_name}, 专长: {tm_specialty}")
...
7. 直接输出消息内容，不要任何标记"""
    ...
    async def get_reminder_with_schedule(self, user_id: str = "default_user") -> Dict[str, Any]:
        ...
        try:
            ...
            tea_master_id = analysis['favorite_tea_master_id']
            duration = analysis.get('favorite_duration', 60)
            ...
            start_hour = max(current_hour + 1, 10)  # 从下一小时开始，但不早于10点

            for hour in range(start_hour, 23):  # 10点到23点营业时间
                check_time = today.replace(hour=hour, minute=0, second=0, microsecond=0)
                end_time = check_time + timedelta(minutes=duration)

                if db.is_tea_master_available(tea_master_id, check_time, end_time):
            ...
            if not available_times:
                ...
                for hour in range(10, 23):  # 10点到23点营业时间
            ...
            return {
                "message": message,
                "tea_master_available_times": available_times
            }
