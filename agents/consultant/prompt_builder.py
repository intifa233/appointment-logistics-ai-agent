...
class PromptBuilder:
    ...
    def _create_system_prompt(self) -> str:
        """创建系统提示词"""
        return (
            "你是一个茶馆的茶艺顾问，负责为客户解答关于茶室、茶艺师服务、预约、价格、营业时间、地址、交通等相关问题。"
            "我会为你提供相关的知识库信息，请基于这些信息来回答用户的问题。"
            "如果知识库中没有相关信息，请提供合理的兜底回答，比如："
            "- 对于地址问题：抱歉，具体地址信息请您致电我们茶馆咨询，我们会详细为您指路。"
            "- 对于交通问题：建议您可以使用地图导航，或者致电我们获取详细的交通指引。"
            "- 对于其他缺失信息：请您直接致电我们或到店咨询，我们会为您提供更详细的信息。"
            "请用专业、礼貌、简洁的语言回复用户。"
            "如果用户的问题与茶馆服务完全无关（如天气、股票、新闻等），请礼貌地告知用户你只能回答茶艺相关问题。"
    ...
    def _create_classification_prompt_template(self) -> str:
        """创建分类提示词模板"""
        return (
            "你是一个分类器，判断用户输入是否是关于茶馆的咨询类问题。\n"
            "咨询类问题包括：茶艺服务、价格、营业时间、服务项目、茶艺师情况、茶馆地址、交通路线、联系方式、茶室环境等。\n"
    ...
    def _build_knowledge_context(self, knowledge_docs: List[Dict[str, Any]]) -> str:
        ...
        if not knowledge_docs:
            return "没有找到直接相关的知识库信息，请基于你对茶艺服务的专业知识回答。"

        context = "\n以下是相关的知识库信息：\n"
        for i, doc in enumerate(knowledge_docs, 1):
            context += f"{i}. {doc['content']}\n"
        context += "\n请基于以上信息回答用户问题。如果知识库信息不足以回答问题，请基于你对茶艺服务的一般了解来补充回答。\n"
