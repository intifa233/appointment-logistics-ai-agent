...
from .repositories import UserBehaviorRepository, TeaMasterRepository
...
class LocalUserBehaviorDB:
    ...
    def record_user_behavior(self, action_type: str, action_data: Optional[Dict[str, Any]] = None,
                            tea_master_id: Optional[int] = None, session_id: Optional[str] = None,
                            user_id: str = 'default_user'):
        """兼容性方法：记录用户行为"""
        return self.repo.record_behavior(user_id, action_type, action_data, tea_master_id, session_id)
    ...
    def create_recommendation(self, recommendation_type: str, content: str,
                            tea_master_id: Optional[int] = None, user_id: str = 'default_user'):
        """兼容性方法：创建推荐"""
        return self.repo.create_recommendation(user_id, recommendation_type, content, tea_master_id)
...
class LocalTeaMasterDB:
    """
    茶艺师数据库的兼容性类

    将旧接口重定向到新的TeaMasterRepository
    """

    def __init__(self, db_path='sqlite:///data/tea_house.db'):
        warnings.warn(
            "LocalTeaMasterDB已弃用，请使用 DatabaseRouter().tea_masters 或 TeaMasterRepository",
            DeprecationWarning,
            stacklevel=2
        )
        self.session_manager = SessionManager(db_path)
        self.repo = TeaMasterRepository(self.session_manager)

    def get_tea_master_by_name(self, session, name: str):
        """兼容性方法：根据姓名获取茶艺师信息"""
        return self.repo.get_tea_master_by_name(name)

    def get_all_tea_masters(self, session):
        """兼容性方法：获取所有茶艺师"""
        return self.repo.get_all_tea_masters()

    def get_all_specialties(self, session):
        """兼容性方法：获取所有茶艺师专长"""
        return self.repo.get_all_specialties()

    def add_tea_master(self, session, name: str, gender: Optional[str] = None, specialty: Optional[str] = None):
        """兼容性方法：添加茶艺师"""
        return self.repo.add_tea_master(name, gender, specialty)

    def add_schedule(self, session, tea_master_id: int, start_time, end_time, status: str, appointment_id: Optional[int] = None):
        """兼容性方法：添加排班"""
        return self.repo.add_schedule(tea_master_id, start_time, end_time, status, appointment_id)

    def get_tea_master_schedules(self, session, tea_master_id: int, date):
        """兼容性方法：获取茶艺师排班"""
        return self.repo.get_tea_master_schedules(tea_master_id, date)

    def is_tea_master_available(self, session, tea_master_id: int, start_time, end_time) -> bool:
        """兼容性方法：检查茶艺师是否可用"""
        return self.repo.is_tea_master_available(tea_master_id, start_time, end_time)

    def get_tea_masters_by_gender(self, session, gender: str):
        """兼容性方法：根据性别获取茶艺师"""
        return self.repo.get_tea_masters_by_gender(gender)


# 为了完全兼容，也提供从原始base_db模块导入的接口
class BaseTeaMasterDB:
    """兼容性基类"""
    pass
