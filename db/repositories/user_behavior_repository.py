...
from ..models import UserBehavior, UserPreference, UserRecommendation, TeaMaster
...
class UserBehaviorRepository(BaseUserBehaviorRepository):
    ...
    def get_user_statistics(self, user_id: str, days_back: int = 30) -> Dict[str, Any]:
        ...
        with self.session_manager.session_scope() as session:
            ...
            favorite_tea_master = session.query(
                UserBehavior.tea_master_id,
                TeaMaster.name,
                func.count(UserBehavior.tea_master_id).label('count')
            ).join(TeaMaster).filter(
                UserBehavior.user_id == user_id,
                UserBehavior.action_type == 'appointment',
                UserBehavior.created_at >= cutoff_date
            ).group_by(UserBehavior.tea_master_id, TeaMaster.name).order_by(
                func.count(UserBehavior.tea_master_id).desc()
            ).first()
            ...
            return {
                ...
                'favorite_tea_master_id': favorite_tea_master[0] if favorite_tea_master else None,
                'favorite_tea_master_name': favorite_tea_master[1] if favorite_tea_master else None,
                'favorite_tea_master_visits': favorite_tea_master[2] if favorite_tea_master else 0,
    ...
    def get_tea_master_popularity(self, days_back: int = 30) -> List[Dict[str, Any]]:
        """
        获取茶艺师受欢迎程度统计
        ...
        with self.session_manager.session_scope() as session:
            ...
            popularity = session.query(
                UserBehavior.tea_master_id,
                TeaMaster.name,
                func.count(UserBehavior.tea_master_id).label('appointment_count'),
    ...
    def _behavior_to_dict(self, behavior: UserBehavior) -> Dict[str, Any]:
        ...
        return {
            ...
            'tea_master_id': behavior.tea_master_id,
            'tea_master_name': behavior.tea_master.name if behavior.tea_master else None,
    ...
    def _recommendation_to_dict(self, recommendation: UserRecommendation) -> Dict[str, Any]:
        ...
        return {
            ...
            'tea_master_id': recommendation.tea_master_id,
            'tea_master_name': recommendation.tea_master.name if recommendation.tea_master else None,
